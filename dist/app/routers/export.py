...
async def export_audio(request: ExportRequest, background_tasks: BackgroundTasks):
    ...
    # Just-In-Time (JIT) Two-Way FFMPEG Scan ONLY for MP3s
    resolved_ffmpeg_path = None
    if request.format == "mp3":
        # 1. Scan System PATH silently
        resolved_ffmpeg_path = shutil.which("ffmpeg")
        
        # 2. Scan Local Bin folder if not in PATH
        ...
        # 3. If both fail, trigger UI download prompt
        if not resolved_ffmpeg_path:
    ...
    def export_task():
        ...
        try:
            ...
            if request.format == "mp3":
                ...
                try:
                    # Use the dynamically resolved path from the JIT scan
                    subprocess.run(
